...
@Component({
  selector: 'app-secondyear-chapterone-partb',
  imports: [MatIconModule, CommonModule, FormsModule],
  templateUrl: './secondyear-chapterone-partb.html',
  styleUrl: './secondyear-chapterone-partb.scss'
})
export class SecondYearChapteronePartb {
...
emilyImage = 'img/emilycheer.png';
  ...
  handleKeyboardEvent(event: KeyboardEvent) {
    ...
    if(event.key == "Enter" && this.winMessage == "WIN!!") {
      this.router.navigate(['second-year/chapterone/partc']);
    }
  ...
  if (value.length > 3) {
    ...
    if(this.plateCount == 915) //please check this
    {
